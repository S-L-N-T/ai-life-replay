...
export interface EventChoice {
  text: string
  effect?: string
  // structured effects generated by backend: [{ key: string, delta: number }]
...
interface GameStore {
  ...
  consumeBufferedEvent: () => YearEvent | null
  clearBufferedEvents: () => void
  makeChoice: (eventIndex: number, choiceIndex: number, customChoice?: string) => void
...
export const useGameStore = create<GameStore>((set, get) => ({
  ...
  selectWorld: (worldId) => {
    ...
    set({
      ...
      review: null,
      gameSession: null,
      phase: worldId === 'custom' ? 'idle' : 'identity',
    })
  },
  ...
  consumeBufferedEvent: () => {
    ...
    set({ bufferedEvents: rest })
    return first
  },

  clearBufferedEvents: () => set({ bufferedEvents: [] }),

  makeChoice: (eventIndex, choiceIndex, customChoice) => {
    const events = get().yearEvents
    if (eventIndex >= events.length) return
    const event = events[eventIndex]
    if (!event.choices) return
    if (choiceIndex >= event.choices.length && choiceIndex !== 3) return

    const updated = [...events]
    updated[eventIndex] = {
      ...event,
      chosenIndex: choiceIndex,
      customChoice: customChoice?.trim() || undefined,
    }
    ...
    // 如果是自定义选项，将文本写回 choice.text
    if (choiceIndex === 3 && customChoice) {
      choice.text = customChoice
    }

    // 合并结构化 effects 或尝试从 effect 文本解析
    let effects: { key: string; delta: number }[] = []
    if (choice.effects && choice.effects.length) {
      effects = choice.effects
    } else if (choice.effect) {
      // 解析简单格式，例如 "intelligence:+1" 或 "智力 +1" 或 "wealth -2"
      ...
      for (const p of parts) {
        ...
        if (m) {
          ...
          // 尝试把中文名映射到属性 key（优先直接使用 rawKey）
          const key = rawKey // mapping could be improved later
          effects.push({ key, delta })
        }
      }
    }

    if (effects.length > 0) {
      // 应用 effects：若 key 在基础属性中则修改 attributes，否则修改 acquiredAttributes
      const attrs = get().attributes.map((a) => ({ ...a }))
      const acquired = { ...get().acquiredAttributes }

      for (const ef of effects) {
        const baseIdx = attrs.findIndex((a) => a.key === ef.key)
        if (baseIdx !== -1) {
          attrs[baseIdx].value = (attrs[baseIdx].value || 0) + ef.delta
        } else {
          acquired[ef.key] = (acquired[ef.key] || 0) + ef.delta
        }
      }

      set({ attributes: attrs, acquiredAttributes: acquired })
    }
  },
